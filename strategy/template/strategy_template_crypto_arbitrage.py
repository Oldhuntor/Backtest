...
class template():
    def __init__(self, startdate, endate, symbolA, symbolB, frequency, exchange, broker: Broker, params):
    ...
    def get_data_sqlite(self):
        ...
        symbol A 主动腿，symbol B 被动腿

        """
        table_nameA = f"{self.exchange}_{self.symbolA}_{self.frequency}"
        table_nameB = f"{self.exchange}_{self.symbolB}_{self.frequency}"
        conn = connectSqlite(DBpath)
        cursor = conn.cursor()

        # 执行 SQL 查询
        query = f"SELECT * FROM {table_nameA}"
        cursor.execute(query)

        # 获取查询结果
        self.dataA = cursor.fetchall()

        # 执行 SQL 查询
        query = f"SELECT * FROM {table_nameB}"
        cursor.execute(query)

        # 获取查询结果
        self.dataB = cursor.fetchall()
